...
/*done-ssr-incremental-rendering-client@1.0.1#dist/global/done-ssr-incremental-rendering-client*/
(function (exports, global, doEval) {
    var origDefine = global.define;
    var get = function (name) {
        var parts = name.split('.'), cur = global, i;
        for (i = 0; i < parts.length; i++) {
            if (!cur) {
                break;
            }
    ...
    var set = function (name, val) {
        var parts = name.split('.'), cur = global, i, part, next;
        for (i = 0; i < parts.length - 1; i++) {
            part = parts[i];
            next = cur[part];
            if (!next) {
    ...
    var modules = global.define && global.define.modules || global._define && global._define.modules || {};
    var ourDefine = global.define = function (moduleName, deps, callback) {
        var module;
        if (typeof deps === 'function') {
            callback = deps;
...
}({}, window, function (__$source__, __$global__) {
    eval('(function() { ' + __$source__ + ' \n }).call(__$global__);');
}));
define('done-ssr-incremental-rendering-client@1.0.1#dist/global/done-ssr-incremental-rendering-client', function (require, exports, module) {
    module.exports = {
        ROUTE: 0,
        TEXT: 1,
        COMMENT: 2,
